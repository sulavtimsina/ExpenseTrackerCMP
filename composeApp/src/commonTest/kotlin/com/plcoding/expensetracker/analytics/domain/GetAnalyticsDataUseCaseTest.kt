...
class GetAnalyticsDataUseCaseTest {
    ...
    @Test
    fun `invoke returns correct analytics data for given period`() = runTest {
        // Given
        val expenses = listOf(
    ...
    @Test
    fun `invoke returns empty data for no expenses`() = runTest {
        // Given
    ...
    @Test
    fun `invoke calculates monthly trends correctly`() = runTest {
        // Given
        val expenses = listOf(
    ...
    @Test
    fun `invoke calculates daily trends correctly`() = runTest {
        // Given
        val expenses = listOf(
            createExpense("1", 100.0, ExpenseCategory.FOOD, LocalDateTime(2024, 1, 1, 10, 0)),
...
class FakeExpenseRepository : ExpenseRepository {
    ...
    override suspend fun getExpenseById(id: String) = 
        com.plcoding.expensetracker.core.domain.Result.Success(expenses.find { it.id == id })

    override suspend fun insertExpense(expense: Expense) = 
        com.plcoding.expensetracker.core.domain.Result.Success(Unit)
    ...
    override fun getExpensesByCategory(category: ExpenseCategory) = 
        flowOf(expenses.filter { it.category == category })

    override fun getExpensesByDateRange(startDate: LocalDateTime, endDate: LocalDateTime) = 
