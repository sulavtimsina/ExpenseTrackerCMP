...
class AuthRepository {
    ...
    val currentUser: Flow<String?> = auth.sessionStatus.map { status ->
        when (status) {
    ...
    suspend fun signUp(email: String, password: String): Result<String, AuthError> {
        return try {
    ...
    suspend fun signIn(email: String, password: String): Result<String, AuthError> {
        return try {
